...
OUTDIR=/projects/ag-waldvogel/pophistory/CRIP/comparison_HB_rho/run01

INPDIR=/projects/ag-waldvogel/pophistory/CRIP/plink_haplotype_blocks
...
for chr in "${CHR[@]}"; do
    cd ${INPDIR}
    awk 'NR>1 {printf "Chr%s\t%d\t%d\n", $1, $2-1, $3}' all_pop_blocks.blocks.det > all_pop_haploblocks.bed
    cat all_pop_haploblocks.bed | grep "${chr}" | sort -k1,1 -k2,2n > "${chr}"_all_pop_haploblocks.bed
done
...
for ind in "${IND[@]}"; do
  for chr in "${CHR[@]}"; do
    ...
    -b ${INPDIR}/"${chr}"_all_pop_haploblocks.bed > \
    "${ind}"_"${chr}"_ismc.rho.1kb.bedgraph_renamed.sorted_HB.bed
  done
done
...
for ind in "${IND[@]}"; do
  for chr in "${CHR[@]}"; do
    cd ${OUTDIR}
    awk '$7 - $6 >= 500' "${ind}"_"${chr}"_ismc.rho.1kb.bedgraph_renamed.sorted_HB.bed > "${ind}"_"${chr}"_ismc.rho.1kb.bedgraph_renamed.sorted_HB_more500bp.bed
  done
done
...
for ind in "${IND[@]}"; do
  for chr in "${CHR[@]}"; do
    cd ${OUTDIR}
    awk '$7 - $6 < 500' "${ind}"_"${chr}"_ismc.rho.1kb.bedgraph_renamed.sorted_HB.bed > "${ind}"_"${chr}"_ismc.rho.1kb.bedgraph_renamed.sorted_HB_less500bp.bed
  done
done
