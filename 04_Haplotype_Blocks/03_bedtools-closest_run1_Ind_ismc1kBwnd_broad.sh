...
OUTDIR=/projects/ag-waldvogel/pophistory/CRIP/comparison_HB_rho/run02
...
for ind in "${IND[@]}"; do
  for chr in "${CHR[@]}"; do
    ...
    "${ind}"_"${chr}"_ismc.rho.1kb.bedgraph_renamed.sorted_HB_broad.bed
  done
done
...
for ind in "${IND[@]}"; do
  for chr in "${CHR[@]}"; do
    cd ${OUTDIR}
    awk '$7 - $6 >= 500' "${ind}"_"${chr}"_ismc.rho.1kb.bedgraph_renamed.sorted_HB_broad.bed > "${ind}"_"${chr}"_ismc.rho.1kb.bedgraph_renamed.sorted_HB_more500bp_broad.bed
  done
done
...
for ind in "${IND[@]}"; do
  for chr in "${CHR[@]}"; do
    cd ${OUTDIR}
    awk '$7 - $6 < 500' "${ind}"_"${chr}"_ismc.rho.1kb.bedgraph_renamed.sorted_HB_broad.bed > "${ind}"_"${chr}"_ismc.rho.1kb.bedgraph_renamed.sorted_HB_less500bp_broad.bed
  done
done
