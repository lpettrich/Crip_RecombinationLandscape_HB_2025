...
for p in "${POP[@]}"; do
    if [[ -f "${OUTDIR}/${p}_snps.vcf.gz" ]]; then
        plink --vcf "${OUTDIR}/${p}_snps.vcf.gz" --make-bed --maf 0.10 --geno 0.05 --out "${OUTDIR}/${p}_filtered" --threads 5
    else
...
plink --vcf "${OUTDIR}/all_pop_snps.vcf.gz" --make-bed --maf 0.10 --geno 0.05 --out "${OUTDIR}/all_pop_filtered"
